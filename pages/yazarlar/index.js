...
import { ALL_AUTHORS_QUERY } from "../../data/dato_posts";
...
export default function blog(props) {
  const { data } = props;
  const blogData = data.allAuthors;
  return (
    <div>
      <Head>
        ...
        <div className="col-span-1 ">
          <div className=" sticky top-32 flex flex-col justify-center p-4 gap-y-1 font-bold items-center border-2 border-indigo-600 rounded-md mt-14 shadow-lg">
            ...
            {blogData.map((post, index ) =>index < 3 &&(
              <a href={`/blog/${post.authorLink}`} key={post.id} >
                <h3 className="mt-5">{post.authorname}</h3>
              </a>
            ))}
          </div>
        </div>
